...
Rkt_Data car(const Rkt_Data& ths)
{
    try {
        throw (ths.t==ths.LST);
    }catch (bool flag){
        if(!flag){
            ...
            return ths.container[0];
        }
    }
}
Rkt_Data cdr(const Rkt_Data& ths)
{
    try {
        throw (ths.t==ths.LST);
    }catch (bool flag){
        ...
        else if(ths.container.size()<2)
        {
            cerr<<"No cdr"<<'\n';
        }
        else{
            Rkt_Data tmp;
            tmp.container=vector<Rkt_Data>(ths.container.begin()+1,ths.container.end());
            return tmp;
        }
    }
}
